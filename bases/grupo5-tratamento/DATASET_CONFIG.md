...
### Quais colunas do arquivo bruto de ouro entram?

De `gold_daily_prices.csv`, o script usa somente `DATE` e `VALUE`. `VALUE` é renomeada para `GOLD_PRICE` no CSV final.

`IS_HOLIDAY` não é usada porque o arquivo recebido não documenta uma regra ou fonte reproduzível para essa coluna. `TARGET_UP` não é usada porque revela se o preço sobe na próxima observação e, portanto, depende do futuro.

### O que é `TARGET`?

`TARGET` é criado pelo script como o próximo `GOLD_PRICE` (`GOLD_PRICE.shift(-1)`). É a resposta que os modelos tentam prever e serve somente para treino e avaliação; nunca deve ser usada como feature.

### O que são as colunas `GOLD_LAG_*`?

São preços observados no passado: `GOLD_LAG_1`, `GOLD_LAG_5` e `GOLD_LAG_20` correspondem a uma, cinco e vinte observações anteriores. As estatísticas móveis também usam somente observações anteriores. Esses campos permitem que os modelos usem histórico sem acessar preços futuros.
